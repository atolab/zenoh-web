...
The new 0.7.0 release of Zenoh, codename Charmander, introduces some long-waited functionalities to Zenoh:
- Mutual TLS authentication;
- MQTT plugin;
- S3 storage backend;

and some new freshly-landed functionalities:
- Query payload;
- C++ binding;
...
That’s why you need to use user and password if you want to authenticate to the service itself. 
In Zenoh, it was the same until now as shown in the figure below. 
As a client or peer you could verify the authenticity of the router by using [TLS](https://zenoh.io/docs/manual/tls/), but then the router would use [user and password authentication](https://zenoh.io/docs/manual/user-password/) to validate the clients.

![TLS - Server authentication](../../img/20220922-blog-zenoh-charmander/tls1.png)

Mutual TLS authentication allows the router to authenticate incoming TLS connections from clients, peers, or other routers as shown in the figure below. In this case, you will no longer need to use user and password authentication! You can [check here](https://zenoh.io/docs/manual/tls/#mutual-authentication-mtls) how to configure mTLS.
...
The [Zenoh MQTT plugin](https://github.com/eclipse-zenoh/zenoh-plugin-mqtt) converts a Zenoh router into a full-fledged MQTT broker supporting MQTT V3 and V5 clients. This allows to exploit all Zenoh routing and storage capabilities from your MQTT client, but also to make Zenoh clients and peers talk with MQTT clients as shown in the figure below! To test it out just follow the [README](https://github.com/eclipse-zenoh/zenoh-plugin-mqtt).

![MQTT plugin](../../img/20220922-blog-zenoh-charmander/mqtt.png)

----
# S3 storage backend
Speaking of Zenoh storage capabilities, [S3 storage backend](https://github.com/eclipse-zenoh/zenoh-backend-s3) has just landed and it can be used with [Amazon S3](https://aws.amazon.com/s3/) and [MinIO](https://min.io/). 
This allows you to plug S3 buckets into the Zenoh infrastructure and start storing and retrieving data from there. To test it out just follow the [README](https://github.com/eclipse-zenoh/zenoh-backend-s3).
...
This allows you to pass more information in the query that can be received and interpreted by the matching queryables. 
...
All in all, this is the very first release of the C++ bindings so the Zenoh team will highly appreciate any help in testing them on various platforms, system architecture, etc. and to report any issue you might encounter. This will help in greatly improving its maturity and robustness.
...
----
# What’s next?
As usual, are you ready to keep rocking with Zenoh? 
