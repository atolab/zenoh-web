...
In a nutshell,
 1. For each Zenoh type in the API, there are in fact two types: an ### owned** (e.g., `z_owned_*_t`) and a **loaned (e.g., `z_*_t`) type. For example, `z_session_t` has an equivalent defined as `z_owned_session_t`. While the former is a borrowed/loaned object (meaning, the user does not need to release it), the latter is owned by the and thus the user is responsible to release it.

 2. A set of ownership management helpers, `z_move`, `z_loan`, `z_check` and `z_drop` are provided to ease the management of owned objects and defines how an object is passed to the API.
   - With `z_move`, the user give the ownership of an owned object to the API, meaning that he no longer owns the object and therefore no more responsible to release it.
...
In the new Zenoh-Pico API, any private function, type or constant is prefixed with a `_z_*` or `_Z_*`. The user ### must not use them, since we will not guarantee their stability or even existence. Users might solely use any function, type or contant that starts with a `z_*` or `Z_*`
...
*zenoh v0.6.x (C99)*
```C
if (z_put(z_session_loan(&s), z_keyexpr_loan(&keyexpr), "value", strlen("value"), NULL) < 0) {
    printf("Put has failed!\n");
}
```
...
options.encoding = Z_ENCODING_PREFIX_TEXT_PLAIN;
options.priority = Z_PRIORITY_DATA;
if (z_put(z_session_loan(&s), z_keyexpr_loan(&keyexpr), "value", strlen("value"), &options) < 0) {
    printf("Put has failed!\n");
}
```
...
z_publisher_put(z_publisher_loan(pub), "value", strlen("value"), NULL);
```
...
if (z_get(z_session_loan(&s), z_keyexpr("key/expression"), "predicate", z_closure_reply_move(&callback), &opts) < 0) {
    printf("Unable to send query.\n");
    exit(-1);
}
```
...
*zenoh-net v0.5.x*
```C
zp_start_read_task(session);
zp_start_lease_task(session);
```
...
*zenoh-net v0.5.x*
```C
zp_stop_read_task(session);
zp_stop_lease_task(session);
```
...
[*zenoh v0.6.0 (C11)*](https://github.com/eclipse-zenoh/zenoh-pico/tree/master/examples/c11)

[*zenoh v0.6.0 (C99)*](https://github.com/eclipse-zenoh/zenoh-pico/tree/master/examples/c99)
